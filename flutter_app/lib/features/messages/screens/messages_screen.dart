...
class _MessagesScreenState extends State<MessagesScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      debugPrint('🔐 Auth Status in Messages Screen:');
      debugPrint('🔐   Authenticated: ${authProvider.isAuthenticated}');
      debugPrint('🔐   User: ${authProvider.currentUser?.fullName ?? "No user"}');
      debugPrint('🔐   Error: ${authProvider.errorMessage ?? "No error"}');
  ...
  List<dynamic> _filterConversations(List<dynamic> conversations) {
    ...
    if (_searchQuery.isNotEmpty) {
      filtered = filtered.where((conversation) {
        final userName = conversation['user']?['first_name']?.toString().toLowerCase() ?? '';
        final userLastName = conversation['user']?['last_name']?.toString().toLowerCase() ?? '';
        final lastMessage = conversation['last_message']?['content']?.toString().toLowerCase() ?? '';
    ...
    if (_selectedFilter != 'all') {
      filtered = filtered.where((conversation) {
        ...
        // For now, we'll use user_type as the filter
        final userType = conversation['user']?['user_type']?.toString().toLowerCase() ?? '';
        return userType.contains(_selectedFilter);
      }).toList();
    }
      ...
      child: ListTile(
        ...
        leading: Stack(
          children: [
            ...
            // Online status would be implemented with WebSocket
  ...
  void _openConversation(Map<String, dynamic> conversation) {
    final userId = conversation['user']?['id']?.toString();
    final userName = '${conversation['user']?['first_name'] ?? ''} ${conversation['user']?['last_name'] ?? ''}'.trim();
    
    if (userId != null) {
      context.push('/messages/$userId?userName=$userName');
    }
  }
        ...
        content: Column(
          ...
          children: [
            ListTile(
              ...
              subtitle: const Text('Search for users to chat with'),
              onTap: () {
                Navigator.pop(context);
            ...
            ListTile(
              ...
              onTap: () {
                Navigator.pop(context);
                context.push('/messages/search?type=doctor');
              },
            ),
            ListTile(
              ...
              onTap: () {
                Navigator.pop(context);
                context.push('/messages/search?type=admin');
              },
            ),
          ],
        ),
